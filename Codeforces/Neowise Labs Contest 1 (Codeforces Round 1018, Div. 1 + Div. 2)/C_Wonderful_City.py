...
def min_cost(cost,mat):
    
    n = len(mat)
    v1,v2,v3 = [0]*(n), [0]*(n), [0 ]*(n)
    for i in range(1,n ):
    ...
    dp1, dp2 = [0]*n , [0]*n  #curr row F , #curr row T
    dp2[0] = cost[0]
    for i in range(1,n):
        if v1[i] == 0 and v2[i] == 0 and v3[i] == 0:
            dp1[i] = min(dp2[i-1], dp1[i-1]   )
            dp2[i] = cost[i] + min(dp1[i-1], dp2[i-1]   )

        elif v1[i] == 0 and v2[i] == 0 and v3[i] == 1 :
            dp1[i] = min( dp1[i-1], dp2[i-1] )
            dp2[i] = cost[i] + dp2[i-1]
        elif v1[i] == 0 and v2[i] == 1 and v3[i] == 0 :
            dp1[i] = dp2[i-1]
            dp2[i] = cost[i] + dp1[i-1]

        elif v1[i] == 0 and v2[i] == 1 and v3[i] == 1 :
            dp1[i] = dp2[i-1]
            dp2[i] = float("inf")

        elif v1[i] == 1 and v2[i] == 0 and v3[i] == 1 :
            dp1[i] = dp1[i-1] 
            dp2[i] = cost[i] + dp2[i-1] 

        elif v1[i] == 1 and v2[i] == 0 and v3[i] == 0 :
            dp1[i] = dp1[i-1] 
            dp2[i] = cost[i] + min(dp1[i-1], dp2[i-1])
...
def solve():
    ...
    # print("v1:",val1)
    # print("v2:",val2)
    if val1 == -1 or val2 == -1 or val1 == float("inf") or val2 == float("inf"):
        print(-1)
    else:
        print(val1+val2)
