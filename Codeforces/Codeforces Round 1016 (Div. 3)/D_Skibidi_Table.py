...
def solve():
    ...
    def fn1(sx,sy,start, sz, x, y):
        if not sz:
            return start
        add_dis = 2**(sz-1)

        x1,y1 = sx,sy
        x1_ ,y1_ = x1 +  add_dis-1, y1 + add_dis-1 
        quad1 = start 

        x2,y2 = x1 + add_dis, y1 + add_dis
        x2_, y2_ = x2 + add_dis - 1 , y2 + add_dis - 1 
        quad2 = quad1 + add_dis*add_dis 

        x3,y3 = x1 + add_dis, y1
        x3_, y3_ = x3 + add_dis - 1 , y3 + add_dis - 1 
        quad3 = quad2 + add_dis*add_dis 

        x4,y4 = x1, y1+add_dis
        x4_, y4_ = x4 + add_dis - 1 , y4 + add_dis - 1 
        quad4 = quad3 + add_dis*add_dis

        if x1 <= x <= x1_ and y1 <= y <= y1_ :
            return fn1(x1,y1,quad1,sz-1,x,y   )

        elif x2 <= x <= x2_ and y2 <= y <= y2_ :
            return fn1(x2,y2,quad2,sz-1,x,y   )
        
        elif x3 <= x <= x3_ and y3 <= y <= y3_ :
            return fn1(x3,y3,quad3,sz-1,x,y   )
        
        else:
            return fn1(x4,y4,quad4,sz-1,x,y   )


    def fn2( sx,sy,start,sz,sv ):
        if not sz:
            return (sx,sy)
        ...
        if quad1 <= sv < quad2  :
            return fn2(x1,y1,quad1,sz-1,sv) 
        elif quad2 <= sv < quad3  :
            return fn2(x2,y2,quad2,sz-1,sv)
        elif quad3 <= sv < quad4  :
            return fn2(x3,y3,quad3,sz-1,sv)
        else :
            return fn2(x4,y4,quad4,sz-1,sv)
    ...
    for _ in range(q):
        ...
        if inp_arr[0] == "->":
            print(fn1(1,1,1,n,int(inp_arr[1]),int(inp_arr[2])))
        else:
            # print("-")
            print(*fn2(1,1,1,n,int(inp_arr[1])))
